...
import { VariableRows, repeated_names } from "../components/VariableRows.jsx";
import { coerce_variable_value } from "../helper/variables.js";
import {
  form_data_to_vars,
    ...
    <div id="start-task">
      ...
      <div class="start-form">
        {params.tab != null ? (
          ...
          <p class="empty-state">
            {t("tasks.start-process.select-definition")}
          </p>
        )}
      </div>
    </div>
...
const StartGeneratedForm = ({ definition_id }) => {
  const state = useContext(AppState),
    ...
    // Variables entered by hand. Only offered when the definition has no start
    // form: with one, the form decides what may be set.
    [own_variables, setOwnVariables] = useState([]),
  ...
  if (
    !no_form &&
    (rendered?.status !== RESPONSE_STATE.SUCCESS || !rendered?.data)
  ) {
  ...
  const on_submit = ({ data, errors }) => {
    ...
    setError(null);
    if (repeated_names(own_variables)) {
      setError(t("tasks.form.duplicate-variable"));
      return;
    }
    const variables = {
      ...form_data_to_vars(data, vars, allowed),
      ...Object.fromEntries(
        own_variables
    ...
    engine_rest.process_definition
      .submit_form(
        state,
        definition_id,
        with_business_key({ variables }, business_key),
      )
      .then((result) => {
  ...
  return (
    <div class="task-form camunda-task-form">
      ...
      {schema.components.length === 0 && (
        <VariableRows rows={own_variables} on_change={setOwnVariables} />
      )}
      <CamundaForm
        schema={schema}
        data={initial_data}
