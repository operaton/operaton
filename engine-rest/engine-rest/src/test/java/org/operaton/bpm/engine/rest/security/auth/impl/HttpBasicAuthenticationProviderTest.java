...
class HttpBasicAuthenticationProviderTest {
  ...
  void testExtractAuthenticatedUserHeaderNotBasic() {
    when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn("NOTBASIC user:password");
    AuthenticationResult result = provider.extractAuthenticatedUser(request, engine);
    assertThat(result.isAuthenticated()).isFalse();
    assertThat(result.getAuthenticatedUser()).isNull();
  }
  ...
  void testExtractAuthenticatedUserNoCredentials() {
    when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn(BASIC_AUTH_HEADER_PREFIX);
    AuthenticationResult result = provider.extractAuthenticatedUser(request, engine);
    assertThat(result.isAuthenticated()).isFalse();
    assertThat(result.getAuthenticatedUser()).isNull();
  }
  ...
  void testExtractAuthenticatedUserValidCredentials() {
    when(identityService.checkPassword(USER_ID, PASSWORD)).thenReturn(true);
  ...
  void testExtractAuthenticatedUserInvalidCredentials() {
    when(identityService.checkPassword(USER_ID, PASSWORD)).thenReturn(false);
    when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn(AUTHORIZATION_HEADER);
    AuthenticationResult result = provider.extractAuthenticatedUser(request, engine);
    assertThat(result.isAuthenticated()).isFalse();
    assertThat(result.getAuthenticatedUser()).isEqualTo(USER_ID);
  ...
  void testExtractAuthenticatedUserInvalidBase64() {
    when(request.getHeader(HttpHeaders.AUTHORIZATION)).thenReturn(BASIC_AUTH_HEADER_PREFIX + "!!!invalidbase64!!!");
    AuthenticationResult result = provider.extractAuthenticatedUser(request, engine);
    assertThat(result.isAuthenticated()).isFalse();
    assertThat(result.getAuthenticatedUser()).isNull();
  }
