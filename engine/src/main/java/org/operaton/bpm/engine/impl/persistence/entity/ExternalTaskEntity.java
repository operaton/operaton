...
public class ExternalTaskEntity implements ExternalTask, DbEntity,
    ...
    if (operatonErrorEventDefinitions != null && !operatonErrorEventDefinitions.isEmpty()) {
      for (OperatonErrorEventDefinition operatonErrorEventDefinition : operatonErrorEventDefinitions) {
        if (errorEventDefinitionMatches(operatonErrorEventDefinition, continueOnException)) {
          bpmnError(operatonErrorEventDefinition.getErrorCode(), errorMessage, null);
          return true;
        }
      }
    }
  ...
  protected boolean errorEventDefinitionMatches(OperatonErrorEventDefinition operatonErrorEventDefinition, boolean continueOnException) {
    try {
      return operatonErrorEventDefinition.getExpression() != null && Boolean.TRUE.equals(operatonErrorEventDefinition.getExpression().getValue(getExecution()));
    } catch (Exception exception) {
      if (continueOnException) {
        ProcessEngineLogger.EXTERNAL_TASK_LOGGER.errorEventDefinitionEvaluationException(id, operatonErrorEventDefinition, exception);
        return false;
      }
      throw exception;
    }
