...
public class EnginePersistenceLogger extends ProcessEngineLogger {
  ...
  public WrongDbException wrongDbVersionException(String version, String dbVersion) {
    return new WrongDbException(exceptionMessage(
      "055",
      "Version mismatch: Operaton library version is '{}' and db version is '{}'. " +
      HINT_TEXT,
      version,
      dbVersion
    ), version, dbVersion);
  ...
  public ProcessEngineException missingActivitiTablesException() {
    return new ProcessEngineException(exceptionMessage(
      "057",
      "There are no Operaton tables in the database. " +
        HINT_TEXT
    ));
  }
  ...
  public ProcessEngineException multipleTenantsForOperatonFormDefinitionKeyException(String operatonFormDefinitionKey) {
    return new ProcessEngineException(exceptionMessage(
        "109",
        "Cannot resolve a unique Operaton Form definition for key '{}' because it exists for multiple tenants.",
        operatonFormDefinitionKey
        ));
  }
