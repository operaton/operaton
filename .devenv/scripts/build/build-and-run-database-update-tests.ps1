...
try {
  ...
  try {
    ...
    if ($executeBuild) {
      Write-Host 'INFO: Building BOMs and engine modules'
      Invoke-Maven -Runner $runner -Arguments @('install', '-f', 'bom')
      Invoke-Maven -Runner $runner -Arguments @('install', '-DskipTests', '-am', '-pl', 'distro/sql-script')
    } else {
    ...
    if ($executeTest) {
      ...
      $arguments = @("-P$($profiles -join ',')", 'clean', 'install', '-f', 'qa')
      Write-Host "INFO: Running database update tests for database $database using profiles: [$($profiles -join ' ')]"
      Invoke-Maven -Runner $runner -Arguments $arguments
      Write-Host 'INFO: Database update tests completed successfully'
    } else {
