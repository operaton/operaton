...
public class ExecutionEntity extends PvmExecutionImpl implements Execution, ProcessInstance, DbEntity, HasDbRevision,
  ...
  protected transient VariableStore<VariableInstanceEntity> variableStore = new VariableStore<>(this,
  ...
  public ExecutionEntity createExecution() {
    ...
    // with the fix of CAM-9249 we presume that the parent and the child have
    // the same startContext
  ...
  public ExecutionEntity createSubProcessInstance(PvmProcessDefinition processDefinition, String businessKey,
    ...
    if (tenantId != null) {
      ...
      // if process definition has no tenant id, inherit this process instance's
      // tenant id
  ...
  public CaseExecutionEntity createSubCaseInstance(CmmnCaseDefinition caseDefinition, String businessKey) {
    ...
    if (tenantId != null) {
      ...
      // if case definition has no tenant id, inherit this process instance's
      // tenant id
      subCase.setTenantId(this.tenantId);
    }

    // manage bidirectional super-process-sub-case-instances relation
    subCase.setSuperExecution(this);
  ...
  public void initializeTimerDeclarations() {
    ...
    for (Map<String, TimerDeclarationImpl> timerDeclarations : TimerDeclarationImpl
        .getTimeoutListenerDeclarationsForScope(scope).values()) {
  ...
  public void start(Map<String, Object> variables, VariableMap formProperties) {
    if (getSuperExecution() == null) {
      ...
      setRootProcessInstanceId(getSuperExecution().getRootProcessInstanceId());
    }
  ...
  public boolean isProcessInstanceStarting() {
    // the process instance can only be starting if it is currently in
    // main-memory already
  ...
  protected void ensureExecutionTreeInitialized() {
    List<ExecutionEntity> executionEntities = Context.getCommandContext().getExecutionManager()
        .findExecutionsByProcessInstanceId(processInstanceId);

    ExecutionEntity procInstance = isProcessInstanceExecution() ? this : null;

    if (procInstance == null) {
      for (ExecutionEntity execution : executionEntities) {
        if (execution.isProcessInstanceExecution()) {
          procInstance = execution;
        }
      }
    }
  ...
  public void restoreProcessInstance(Collection<ExecutionEntity> executions,
    ...
    Map<String, ExecutionEntity> executionsMap = determineExecutionsById(executions);
    Map<String, List<VariableInstanceEntity>> variablesByScope = determineVariablesByScope(variables);

    restoreExecutionTree(executions, eventSubscriptions, variables, executionsMap, variablesByScope);
    restoreEventSubscriptions(eventSubscriptions, executionsMap);
    restoreJobs(jobs, executionsMap);
    restoreTasks(variables, tasks, executionsMap, variablesByScope);
    restoreIncidents(incidents, executionsMap);
    restoreExternalTasks(externalTasks, executionsMap);
  }

  private Map<String, ExecutionEntity> determineExecutionsById(Collection<ExecutionEntity> executions) {
  ...
  private Map<String, List<VariableInstanceEntity>> determineVariablesByScope(
  ...
  public IncidentEntity getIncidentByCauseIncidentId(String causeIncidentId) {
    return getIncidents().stream()
        .filter(incidentEntity -> Objects.equals(incidentEntity.getCauseIncidentId(), causeIncidentId)).findAny()
        .orElse(null);
  }
  ...
  public int getCachedEntityState() {
    ...
    cachedEntityState = BitMaskUtil.setBit(cachedEntityState, EXTERNAL_TASKS_BIT,
        externalTasks == null || !externalTasks.isEmpty());
  ...
  public String getRootProcessInstanceId() {
    if (isProcessInstanceExecution()) {
      ...
      return getProcessInstance().rootProcessInstanceId;
    }
  }
