...
import org.operaton.bpm.webapp.neo.impl.security.filter.headersec.provider.impl.ContentSecurityPolicyProvider;
import org.operaton.bpm.webapp.neo.impl.security.filter.headersec.provider.impl.ContentTypeOptionsProvider;
import org.operaton.bpm.webapp.neo.impl.security.filter.headersec.provider.impl.StrictTransportSecurityProvider;
import org.operaton.bpm.webapp.neo.impl.security.filter.headersec.provider.impl.XssProtectionProvider;
    ...
    if (response instanceof HttpServletResponse httpResponse) {
      for (HeaderSecurityProvider provider : headerSecurityProviders) {
        ...
        if (!provider.isDisabled()) {
          ...
          String headerValue = provider.getHeaderValue(request);
