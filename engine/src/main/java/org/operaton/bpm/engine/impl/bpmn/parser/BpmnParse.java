...
public class BpmnParse extends Parse {
  ...
  protected void addEventSubscriptionDeclaration(EventSubscriptionDeclaration subscription, ScopeImpl scope, Element element) {
    ...
    if (hasMultipleMessageEventDefinitionsWithSameName(subscription, eventDefinitions.values())){
      addError("Cannot have more than one message event subscription with name '%s' for scope '".formatted(subscription.getUnresolvedEventName()) + scope.getId() + "'",
    ...
    if (hasMultipleSignalEventDefinitionsWithSameName(subscription, eventDefinitions.values())){
      addError("Cannot have more than one signal event subscription with name '%s' for scope '".formatted(subscription.getUnresolvedEventName()) + scope.getId() + "'",
          element, subscription.getActivityId());
    }
    ...
    if (activity.isAsyncAfter()) {
      for (PvmTransition transition : activity.getOutgoingTransitions()) {
        if (transition.getId() == null) {
          addError("Sequence flow with sourceRef='%s' must have an id, activity with id '".formatted(activity.getId()) + activity.getId() + "' uses 'asyncAfter'.",
              null, activity.getId());
        }
      }
    }
  ...
  public void validateExclusiveGateway(ActivityImpl activity) {
    if (activity.getOutgoingTransitions().isEmpty()) {
      ...
      if (condition != null) {
        addError("Exclusive Gateway '%s' has only one outgoing sequence flow ('".formatted(activity.getId()) + flow.getId()
      ...
      for (PvmTransition flow : activity.getOutgoingTransitions()) {
        ...
        if (hasConditon && isDefaultFlow) {
          addError("Exclusive Gateway '%s' has outgoing sequence flow '".formatted(activity.getId()) + flow.getId()
      ...
      if (hasDefaultFlow || flowsWithoutCondition.size() > 1) {
        ...
        for (PvmTransition flow : flowsWithoutCondition) {
          addError(
              "Exclusive Gateway '%s' has outgoing sequence flow '".formatted(activity.getId()) + flow.getId() + "' without condition which is not the default flow.",
        ...
        addWarning(
            "Exclusive Gateway '%s' has outgoing sequence flow '".formatted(activity.getId()) + flow.getId()
  ...
  protected void parseIntermediateLinkEventCatchBehavior(Element intermediateEventElement, ActivityImpl activity, Element linkEventDefinitionElement) {
    ...
    if (eventLinkTargets.containsKey(linkName)) {
      ...
      if (!linkName.equals(elementName)) {
        ...
        addWarning("Link Event named '%s' contains link event definition with name '".formatted(elementName) + linkName
    ...
    if (activityRef != null && scopeElement.findActivityAtLevelOfSubprocess(activityRef) == null) {
      ...
      if (scopeElement.findActivityAtLevelOfSubprocess(activityRef) == null) {
        ...
        scopeElement.addToBacklog(activityRef, () ->
            addError("Invalid attribute value for 'activityRef': no activity with id '%s' in scope '".formatted(activityRef) + scopeId + "'",
  ...
  public void parseServiceTaskLike(
    ...
    if (type != null) {
      if ("mail".equalsIgnoreCase(type)) {
        ...
        addError("Invalid usage of type attribute on %s: '".formatted(elementName) + type + "'", serviceTaskElement);
      }
  ...
  protected void validateServiceTaskLike(
    ...
    if (activity.getActivityBehavior() == null) {
      addError("One of the attributes 'class', 'delegateExpression', 'type', "
          + "or 'expression' is mandatory on %s. If you are using a connector, make sure the ".formatted(elementName)
    ...
    if (priorityAttributeValue == null) {
      ...
      if (!StringUtil.isExpression(priorityAttributeValue)) {
        ...
        try {
          ...
          addError("Value '%s' for attribute '".formatted(priorityAttributeValue) + priorityAttribute + "' is not a valid number", element);
        }
      }
  ...
  protected void validateFieldDeclarationsForShell(Element serviceTaskElement, List<FieldDeclaration> fieldDeclarations) {
    ...
    for (FieldDeclaration fieldDeclaration : fieldDeclarations) {
      ...
      if (("wait".equals(fieldName) || "redirectError".equals(fieldName) || "cleanEnv".equals(fieldName)) && !TRUE.equalsIgnoreCase(fieldValue)
        ...
        addError("undefined value for shell %s parameter :".formatted(fieldName) + fieldValue, serviceTaskElement);
      }
  ...
  protected String getStringValueFromAttributeOrElement(String attributeName, String elementName, Element element, String ancestorElementId) {
    ...
    if (attributeValue != null && childElement != null) {
      addError("Can't use attribute '%s' and element '".formatted(attributeName) + elementName + "' together, only use one", element, ancestorElementId);
    } else if (childElement != null) {
      stringElementText = childElement.getText();
      if (stringElementText == null || stringElementText.isEmpty()) {
        addError("No valid value found in attribute '%s' nor element '".formatted(attributeName) + elementName + "'", element, ancestorElementId);
      } else {
        // Use text of element
  ...
  protected FormDefinition parseFormDefinition(Element flowNodeElement) {
    ...
    if(formRefAttribute != null) {
      ...
      if (formRefBindingAttribute == null || !ALLOWED_FORM_REF_BINDINGS.contains(formRefBindingAttribute)) {
        addError("Invalid element definition: value for formRefBinding attribute has to be one of %s but was ".formatted(ALLOWED_FORM_REF_BINDINGS) + formRefBindingAttribute, flowNodeElement);
      }
  ...
  protected void parseHumanPerformer(Element taskElement, TaskDefinition taskDefinition) {
    ...
    if (humanPerformerElements.size() > 1) {
      addError("Invalid task definition: multiple %s sub elements defined for ".formatted(HUMAN_PERFORMER) + taskDefinition.getNameExpression(), taskElement);
    ...
    for (EscalationEventDefinition existingEscalationEventDefinition : catchingScope.getProperties().get(BpmnProperties.ESCALATION_EVENT_DEFINITIONS)) {
      ...
      if (existingEscalationEventDefinition.getEscalationHandler().isSubProcessScope()
        ...
        if (existingEscalationEventDefinition.getEscalationCode() == null && escalationEventDefinition.getEscalationCode() == null) {
          ...
          addError("multiple escalation event subprocesses with the same escalationCode '%s' are not supported on same scope".formatted(escalationEventDefinition.getEscalationCode()), element, escalationElementId);
        }
        ...
        if (existingEscalationEventDefinition.getEscalationCode() == null && escalationEventDefinition.getEscalationCode() == null) {
          ...
          addError("multiple escalation boundary events with the same escalationCode '%s' are not supported on same scope".formatted(escalationEventDefinition.getEscalationCode()), element, escalationElementId);
        }
      }
    }
    ...
    if (conditionExprElement != null) {
      ...
      for (String variableEvent : variableEventsList) {
        if (!VARIABLE_EVENTS.contains(variableEvent)) {
          addWarning("Variable event: %s is not valid. Possible variable change events are: ".formatted(variableEvent) + Arrays.toString(VARIABLE_EVENTS.toArray()),
  ...
  protected void parseVersion(Element callingActivityElement, ActivityImpl activity, BaseCallableElement callableElement, String bindingAttributeName, String versionAttributeName) {
    ...
    if (binding != null && binding.equals(CallableElementBinding.VERSION) && version == null) {
      addError("Missing attribute '%s' when '".formatted(versionAttributeName) + bindingAttributeName + "' has value '%s'".formatted(CallableElementBinding.VERSION.getValue()), callingActivityElement);
    }
  ...
  protected void parseVersionTag(Element callingActivityElement, ActivityImpl activity, BaseCallableElement callableElement, String bindingAttributeName, String versionTagAttributeName) {
    ...
    if (binding != null && binding.equals(CallableElementBinding.VERSION_TAG) && versionTag == null) {
      addError("Missing attribute '%s' when '".formatted(versionTagAttributeName) + bindingAttributeName + "' has value '%s'".formatted(CallableElementBinding.VERSION_TAG.getValue()), callingActivityElement);
    }
  ...
  public void parseProperty(Element propertyElement, ActivityImpl activity) {
    ...
    if (name == null) {
      if (id == null) {
        addError("Invalid property usage on line %s: no id or name specified.".formatted(propertyElement.getLine()), propertyElement, activity.getId());
      } else {
        name = id;
      }
    }
    ...
    for (Element sequenceFlowElement : processElement.elements("sequenceFlow")) {
      ...
      if (eventLinkSources.containsKey(destinationRef)) {
        ...
        if (destinationRef == null) {
          addError("sequence flow points to link event source with name '%s' but no event target with that name exists. Most probably your link events are not configured correctly.".formatted(linkName), sequenceFlowElement);
      ...
      if ((sourceActivity == null && compensationHandlers.containsKey(sourceRef))
        ...
        addError("Invalid outgoing sequence flow of compensation activity '%s'. A compensation activity should not have an incoming or outgoing sequence flow.".formatted(sourceRef),
            sequenceFlowElement,
            sourceRef,
            id);
        ...
        addError("Invalid incoming sequence flow of compensation activity '%s'. A compensation activity should not have an incoming or outgoing sequence flow.".formatted(destinationRef),
            sequenceFlowElement,
            destinationRef,
            id);
      } else if (sourceActivity == null) {
        addError("Invalid source '%s' of sequence flow '".formatted(sourceRef) + id + "'", sequenceFlowElement);
      } else if (destinationActivity == null) {
        addError("Invalid destination '%s' of sequence flow '".formatted(destinationRef) + id + "'", sequenceFlowElement);
        ...
        addError("Invalid incoming sequenceflow for intermediateCatchEvent with id '%s' connected to an event-based gateway.".formatted(destinationActivity.getId()),
            sequenceFlowElement);
      ...
      if (sequenceFlows != null && sequenceFlows.containsKey(sequenceFlowId)) {
        ...
        if (waypointElements.size() >= 2) {
          ...
          addError("Minimum 2 waypoint elements must be definted for a 'BPMNEdge'", bpmnEdgeElement);
                                                         ...
                                                         // reference
                                                         // 'something'
        addError("Invalid reference in 'bpmnElement' attribute, sequenceFlow %snot found".formatted(sequenceFlowId), bpmnEdgeElement);
      }
    } else {
  ...
  public Double parseDoubleAttribute(Element element, String attributeName, String doubleText, boolean required) {
    if (required && (doubleText == null || "".equals(doubleText))) {
      ...
      try {
        ...
        addError("Cannot parse %s: ".formatted(attributeName) + e.getMessage(), element);
      }
    }
    return -1.0;
  }
  ...
  protected void ensureNoExpressionInMessageStartEvent(Element element,
    ...
    if (eventNameContainsExpression) {
      ...
      addError("Invalid message name '%s' for element '".formatted(messageStartName) +
