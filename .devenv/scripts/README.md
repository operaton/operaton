...
- `--webapps-only` - Shortcut for `--changed-modules=webapps/assembly`. Useful when only the webapps frontend/backend changed.
- `--changed-modules=<dir,dir,...>` - Affected-modules build. Runs up to three Maven passes:
  1. `install -pl <relevant test-jar producers> -am -DskipTests [-Dskip.frontend.build=true]` — only if any are relevant to this change. A few modules (`spin/core`, `engine-cdi`, …) attach a test-jar that is a real compile-time dependency elsewhere; `maven.test.skip` (step 2) makes the jar plugin skip packaging it entirely (not even an empty jar), so these are built for real first (tests compiled and packaged, only *execution* skipped, hence `-DskipTests` here and not `maven.test.skip`) and installed before anything can rely on a missing artifact. Narrowed via `prepare_build.py --list-test-jar-producers=<dirs>` to producers whose test-jar is actually reachable from the affected closure — e.g. a `clients/java/client`-only change needs none of them, so this step is skipped entirely.
  ...
  Steps 1–2 also drop the `check-api-compatibility` Maven profile (which binds the `clirr-maven-plugin` API-compatibility check in ~17 public-API modules) — neither step runs any tests or represents a real verification of the change, so the check is pure overhead there. Step 3 keeps it, since that's the pass that actually verifies the changed modules.
