...
/**
 * Integration test verifying that when {@code operaton.bpm.webapp.session-cookie-path-enforcement=true}
 * is set, the {@code SessionCookiePathFilter} enforces {@code Path=/operaton} on the default
 * {@code JSESSIONID} session cookie.
 */
...
@DirtiesContext
class SessionCookiePathEnforcementEnabledIT {
  ...
  void shouldEnforcePathOnDefaultJSessionId() {
    ...
    assertThat(sessionCookieValue)
        .as("Expected JSESSIONID cookie with enforced Path=/operaton when session-cookie-path-enforcement=true")
        .isNotNull()
        .isNotEmpty()
