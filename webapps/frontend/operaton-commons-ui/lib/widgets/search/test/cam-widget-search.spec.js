...
describe('Search Widget', function () {
  ...
  it('should not show a dropdown at initialization', function () {
    expect(
      page.example('example1').inputDropdown().isDisplayed(),
  ...
  it('should store valid searches in the URL', function () {
    ...
    browser.getCurrentUrl().then(function (url) {
      browser.get(url);
      expect(page.example('example1').searchPills().count()).to.eventually.eql(
        1,
      );
    });
  });
  ...
  it('should adjust searches on changes in the URL', function () {
    ...
    browser.getCurrentUrl().then(function (url) {
      ...
      browser.setLocation(location).then(function () {
        expect(
          page.example('example1').searchPill(0).valueField().text(),
  ...
  it('should retail invalid searches when adjusting searches on changes in the URL', function () {
    ...
    browser.getCurrentUrl().then(function (url) {
      ...
      browser.setLocation(location).then(function () {
        expect(
          page.example('example1').searchPills().count(),
        ).to.eventually.eql(2);
      });
    });
  });
  ...
  it('should use default type', function () {
    ...
    expect(
      page.example('example1').searchPill(0).typeField().text(),
  ...
  it('should store valid searches for multiple widget instances', function () {
    ...
    browser.getCurrentUrl().then(function (url) {
      ...
      expect(
        page.example('example2').searchPill(0).valueField().text(),
  ...
  describe('Groups', function () {
    ...
    it('should show only matching options in the input dropdown', function () {
      ...
      expect(
        page.example('example2').inputDropdownOption(0).getText(),
      ).to.eventually.eql('A');
      expect(
        page.example('example2').inputDropdownOption(1).getText(),
      ).to.eventually.eql('C');
    });
  ...
  describe('local persitence', function () {
    it('is not accessible when the widget does not have search pill and no record can be found', function () {
      expect(
        page.example('example1').storageDropdownButton().isEnabled(),
      ).to.eventually.eql(false);
    });
    ...
    it('allows to save a set of search criteria', function () {
      browser.executeScript('localStorage.removeItem("operaton")');
      ...
      browser
        .executeScript('return JSON.parse(localStorage.getItem("operaton"))')
    ...
    describe('with groups', function () {
      ...
      it('stores the sets in separate groups', function () {
        browser
          .executeScript('localStorage.removeItem("operaton")')
          .then(function (res) {
            ...
            browser
              .executeScript(
                'return JSON.parse(localStorage.getItem("operaton"))',
              )
              .then(function (result) {
