...
const update_tenant = (state, tenant_id, body) =>
  PUT(`/tenant/${encode_id(tenant_id)}`, body, state, state.api.tenant.update);

const delete_tenant = (state, tenant_id) =>
  DELETE(`/tenant/${encode_id(tenant_id)}`, {}, state, state.api.tenant.delete);

// Members of a tenant (used on the tenant details page).
const get_tenant_users = (state, tenant_id) =>
  GET(
    `/user?memberOfTenant=${encode_id(tenant_id)}&firstResult=0&maxResults=50`,
    state,
    state.api.tenant.user_members,
  );

const get_tenant_groups = (state, tenant_id) =>
  GET(
    `/group?memberOfTenant=${encode_id(tenant_id)}&firstResult=0&maxResults=50`,
    state,
    state.api.tenant.group_members,
  );

const add_user_to_tenant = (state, tenant_id, user_name) =>
  PUT(
    `/tenant/${encode_id(tenant_id)}/user-members/${encode_id(user_name)}`,
    {
      id: tenant_id,
      userId: user_name,
    },
    state,
    state.api.tenant.add_user,
  );
...
const remove_user_from_tenant = (state, tenant_id, user_name) =>
  DELETE(
    ...
    state,
    state.api.tenant.remove_user,
  );

const add_group_to_tenant = (state, tenant_id, group_id) =>
  PUT(
    `/tenant/${encode_id(tenant_id)}/group-members/${encode_id(group_id)}`,
    {
      id: tenant_id,
      groupId: group_id,
    },
    state,
    state.api.tenant.add_group,
  );
