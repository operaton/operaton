...
/**
 * Integration test verifying that when {@code operaton.bpm.webapp.session-cookie-path-enforcement}
 * is absent (default false), the {@code SessionCookiePathFilter} is not active and the session
 * cookie {@code Path} attribute is not rewritten.
 */
...
class SessionCookiePathEnforcementDisabledIT {
  ...
  void shouldNotOverrideCookiePathWhenEnforcementDisabled() {
    ...
    assertThat(rawSetCookie)
        .as("Expected no Path=/operaton enforcement when session-cookie-path-enforcement is absent/false")
        .isNotNull()
        .isNotEmpty()
