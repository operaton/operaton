...
describe("TasksPage", () => {
  ...
  // The previous Tasklist showed task variables beside the form and let the
  // assignee change them (disableAddVariableButton in its form plugin).
  describe("task variables", () => {
    const open_variables = () => {
      mockParams = { task_id: "t1", tab: "variables" };
    };

    it("lists what the task carries", () => {
      open_variables();
      signal_response(state.api.task.one, sample_task({ assignee: "alice" }));
      signal_response(state.api.task.variables, {
    ...
    it("says so when there are none", () => {
      ...
      signal_response(state.api.task.variables, {});
      const { getByText } = renderPage(state);
      expect(getByText("tasks.variables.empty")).toBeTruthy();
    });

    it("offers no editing to someone who does not hold the task", () => {
      open_variables();
      ...
      const { queryByText, getByText } = renderPage(state);
      expect(queryByText("tasks.variables.add")).toBeNull();
      expect(getByText("tasks.form.claim-first")).toBeTruthy();
    });

    it("adds a variable with the chosen type", async () => {
      open_variables();
      state.auth.user.id.value = "alice";
      signal_response(state.api.task.one, sample_task({ assignee: "alice" }));
      signal_response(state.api.task.variables, {});
      ...
      fireEvent.input(container.querySelector("#new-variable-name"), {
        target: { value: "amount" },
      });
      fireEvent.change(container.querySelector("#new-variable-type"), {
        target: { value: "Integer" },
      });
      fireEvent.input(container.querySelector("#new-variable-value"), {
        target: { value: "42" },
      });
      fireEvent.click(getByText("tasks.variables.save"));
