...
public class ClassPathProcessApplicationScanner implements ProcessApplicationScanner {
  ...
  public void scanPaResourceRootPath(final ClassLoader classLoader,
                                     ...
                                     final String paResourceRootPath,
  ...
  public void scanPaResourceRootPath(final ClassLoader classLoader,
    ...
    if (paResourceRootPath == null || paResourceRootPath.startsWith("pa:")) {
      String strippedPath = stripPath(paResourceRootPath, "pa:");
      scanUrl(metaFileUrl, strippedPath, true, additionalResourceSuffixes, resourceMap);
    } else {
      String strippedPath = stripPath(paResourceRootPath, "classpath:");
  ...
  private String stripPath(String paResourcePath, String prefixToRemove) {
    if (paResourcePath == null) {
      return null;
    }

    String strippedPath = paResourcePath.replace(prefixToRemove, "");
  ...
  private void scanUrl(URL url,
    ...
    List<String> protocols = Arrays.asList("file:", "jar:", "wsjar:", "zip:");

    if (protocols.stream().anyMatch(urlPath::startsWith)) {
      urlPath = url.getPath();
  ...
  private void scanPath(String urlPath,
                        String paResourceRootPath,
                        boolean isPaLocal,
                        String[] additionalResourceSuffixes,
                        Map<String, byte[]> resourceMap) {
    ...
    if (file.isDirectory()) {
      ...
      handleDirectory(file, rootPath, paResourceRootPath, paResourceRootPath, isPaLocal, additionalResourceSuffixes,
          resourceMap);
    } else {
  ...
  private void handleDirectory(File directory,
    ...
    for (File path : paths) {
      // if it is not PA-local, we have already used the classloader to specify the root path explicitly.
      if (isPaLocal && currentPathSegment != null && !currentPathSegment.isEmpty()) {
        scanPaLocalPath(rootPath, localPath, paResourceRootPath, isPaLocal, additionalResourceSuffixes, resourceMap,
            currentPathSegment, path);
      } else { // at resource root or below -> continue scanning
        scanResourceRoot(rootPath, localPath, paResourceRootPath, isPaLocal, additionalResourceSuffixes, resourceMap,
            paths, path);
      }
    }
  }

  private void scanPaLocalPath(String rootPath, String localPath, String paResourceRootPath, boolean isPaLocal,
      String[] additionalResourceSuffixes, Map<String, byte[]> resourceMap, String currentPathSegment, File path) {
    // only descend into directory, if below resource root:
    if (path.isDirectory() && path.getName().equals(currentPathSegment)) {
      handleDirectory(path, rootPath, localPath, paResourceRootPath, isPaLocal, additionalResourceSuffixes,
          resourceMap);
    }
  }

  private void scanResourceRoot(String rootPath, String localPath, String paResourceRootPath, boolean isPaLocal,
      String[] additionalResourceSuffixes, Map<String, byte[]> resourceMap, File[] paths, File path) {
    ...
    if (!path.isDirectory() && ProcessApplicationScanningUtil.isDeployable(modelFileName,
      ...
      addResource(path, resourceMap, paResourceRootPath, updateResourceFileName(modelFileName, rootPath));
      findDiagramsInPaths(rootPath, paResourceRootPath, resourceMap, path, paths, modelFileName);
  ...
  private void findDiagramsInPaths(String rootPath,
                                   String paResourceRootPath,
                                   Map<String, byte[]> resourceMap,
                                   File path,
                                   File[] paths,
    ...
    for (File file : paths) {
      ...
      if (!path.isDirectory() && ProcessApplicationScanningUtil.isDiagram(diagramFileName, modelFileName)) {
        addResource(file, resourceMap, paResourceRootPath, updateResourceFileName(diagramFileName, rootPath));
      }
    }
  }

  private String updateResourceFileName(String resourceFileName, String rootPath) {
    return resourceFileName.replace(rootPath, "").replace("\\", "/");
  }
  ...
  private void handleArchive(File file,
    ...
    try (ZipFile zipFile = new ZipFile(file)) {
      ...
      while (entries.hasMoreElements()) {
        ...
        handleArchiveEntry(file, paResourceRootPath, additionalResourceSuffixes, resourceMap, zipEntry, zipFile);
      }
    } catch (IOException e) {
  ...
  private void handleArchiveEntry(File file,
                                  String paResourceRootPath,
                                  String[] additionalResourceSuffixes,
                                  Map<String, byte[]> resourceMap,
                                  ZipEntry zipEntry,
    ...
    if (!ProcessApplicationScanningUtil.isDeployable(modelFileName, additionalResourceSuffixes) || !isBelowPath(
        modelFileName, paResourceRootPath)) {
      return;
    }
  ...
  private void findResourcesForArchiveEntry(File file,
                                            String paResourceRootPath,
                                            Map<String, byte[]> resourceMap,
                                            ZipEntry zipEntry,
                                            ZipFile zipFile,
    ...
    while (entries2.hasMoreElements()) {
      ...
      if (ProcessApplicationScanningUtil.isDiagram(diagramFileName, modelFileName)) {
        diagramFileName = prepareResourceNameFromArchiveEntry(diagramFileName, paResourceRootPath);
        addResource(zipFile.getInputStream(zipEntry), resourceMap, file.getName() + "!", diagramFileName);
      }
    }
  }

  private String prepareResourceNameFromArchiveEntry(String baseName, String paResourceRootPath) {
    if (paResourceRootPath != null && !paResourceRootPath.isEmpty()) {
      return baseName.replaceFirst(paResourceRootPath, "");
    }

    return baseName;
  }

  private void addResource(Object source,
                           Map<String, byte[]> resourceMap,
  ...
  private Enumeration<URL> loadClasspathResourceRoots(final ClassLoader classLoader,
  ...
  private boolean isBelowPath(String processFileName, String paResourceRootPath) {
    return paResourceRootPath == null || paResourceRootPath.isEmpty() || processFileName.startsWith(paResourceRootPath);
  }
}
