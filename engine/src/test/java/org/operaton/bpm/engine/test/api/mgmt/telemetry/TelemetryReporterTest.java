...
public class TelemetryReporterTest {
  ...
  public static String DMN_FILE = "org/operaton/bpm/engine/test/api/mgmt/metrics/ExecutedDecisionElementsTest.dmn11.xml";
  ...
  @Test
  @WatchLogger(loggerNames = {"org.operaton.bpm.engine.telemetry"}, level = "DEBUG")
  ...
  public void shouldSendCompleteTelemetryData() {
    ...
    LicenseKeyDataImpl licenseKey = new LicenseKeyDataImpl("customer a", "UNIFIED", "2029-09-01", false, Collections.singletonMap("operatonBPM", "true"), "raw license");

    ProcessEngineConfigurationImpl configuration = createEngine(true);

    configuration.getRepositoryService().createDeployment().addClasspathResource("org/operaton/bpm/engine/test/api/oneTaskProcess.bpmn20.xml").deploy();

    configuration.getTelemetryRegistry().setLicenseKey(licenseKey);
    configuration.getTelemetryData().getProduct().getInternals().getOperatonIntegration().add(OperatonIntegration.SPRING_BOOT_STARTER);
    ...
    TelemetryDataImpl expectedData = simpleData(b -> {
      ...
      b.productName("Operaton BPM Runtime");
      ...
      b.addIntegration(OperatonIntegration.SPRING_BOOT_STARTER);
  ...
  public void shouldSendTelemetryWithOverriddenLicenseInfo() {
    ...
    managementService.toggleTelemetry(true);
    // set key after initialization
    LicenseKeyDataImpl firstLicenseKey = new LicenseKeyDataImpl("customer a", "UNIFIED", "2029-09-01", false, Collections.singletonMap("operatonBPM", "true"), "raw license");
  ...
  @Deployment(resources = {
      "org/operaton/bpm/engine/test/dmn/businessruletask/DmnBusinessRuleTaskTest.testDecisionRef.bpmn20.xml",
      "org/operaton/bpm/engine/test/dmn/businessruletask/DmnBusinessRuleTaskTest.testDecisionOkay.dmn11.xml" })
  ...
  public void shouldRecordUserOperationLog() {
    ...
    assertThat(entry.getNewValue()).isEqualTo("operaton.telemetry.enabled");
  }
  ...
  @Test
  public void shouldSendDataWithOperatonIntegration() {
    // given
    TelemetryDataImpl expectedData = simpleData(
  ...
  protected static class TelemetryDataBuilder {
    ...
    public TelemetryDataBuilder addIntegration(String integration) {
      if(data.getProduct().getInternals().getOperatonIntegration() == null) {
        data.getProduct().getInternals().setOperatonIntegration(new HashSet<>());
      }
      data.getProduct().getInternals().getOperatonIntegration().add(integration);
      return this;
    }
  ...
  protected BpmnModelInstance createProcessWithBusinessRuleTask(String processId, String decisionRef) {
    ...
    task.setOperatonDecisionRef(decisionRef);
