...
public class TimerDeclarationImpl extends JobDeclaration<ExecutionEntity, TimerEntity> {
  ...
  public String resolveAndSetDuedate(ExecutionEntity context, TimerEntity job, boolean creationDateBased) {
    ...
    if (dueDateValue instanceof String string) {
      ...
      throw new ProcessEngineException("Timer '%s' was not configured with a valid duration/time, either hand in a java.util.Date or a String in format 'yyyy-MM-dd'T'hh:mm:ss'".formatted(context.getActivityId()));
    }

    if (duedate==null) {
      if (creationDateBased) {
        if (job.getCreateTime() == null) {
          throw new ProcessEngineException("Timer '%s' has no creation time and cannot be recalculated based on creation date. Either recalculate on your own or trigger recalculation with creationDateBased set to false.".formatted(context.getActivityId()));
        }
